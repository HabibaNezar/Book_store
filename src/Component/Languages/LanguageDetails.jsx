...
export default function LanguageDetails() {
    ...
    function getBooksByLang() {
        return axios.get(`https://www.googleapis.com/books/v1/volumes?q=subject:Science+Fiction&langRestrict=${lang}&key=AIzaSyAkmjT7KZ4hNIlPsZKVlfZTiUMjJfO7mlI`)
    }

    const { data, isLoading, error } = useQuery({
        queryKey: ['booksbyLang', lang],
        queryFn: getBooksByLang,
    })

    if (isLoading) return <p>Loading...</p>
    if (error) return <p>Error fetching books for {lang}</p>

    return (
        <div className='bg-[--background-color]'>
            <div className='md:mx-28'>
                <h2 className='text-[--text-color] text-5xl py-2 font-medium'>
                    Discover Our <span className='text-[--accent-color]'>{lang}</span> Books
                </h2>
