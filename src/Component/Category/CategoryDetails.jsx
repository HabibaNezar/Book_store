...
export default function CategoryDetails() {
    ...
    function getBooksByCategory(){
        return axios.get(`https://www.googleapis.com/books/v1/volumes?q=subject:${title}&maxResults=8&key=${API_KEY}`)
    }

    const { data, error, isLoading } = useQuery({
        queryKey: ['booksByCategory', title],
        ...
        <div className='bg-[--background-color]'>
            <div className=' md:mx-28'>
                <h2 className='text-[--text-color] text-5xl py-2 font-medium'>
                    Discover Our <span className='text-[--accent-color]'>{title}</span> Books
                </h2>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-2 gap-6 my-8">
                    {data?.data?.items?.map((book, index) => (
                        <NavLink to={`/bookDetails/${book.id}`}>
                            <div key={index} className="bg-white p-4 rounded shadow h-[400px] flex flex-col justify-between border-2 border-[--accent-color] md:m-0 m-10">
